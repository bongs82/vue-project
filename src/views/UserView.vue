...
const addMember = async () => {
  ...
  if (!newMemberIdValid.value || !newMemberPasswordValid.value || !newMemberNameValid.value || !newMemberPhoneValid.value) {
  ...
  try {
    ...
    await setDoc(doc(db, "member", "user"), {
  ...
  <div class="user">
    ...
    <div class="form-section">
      ...
      <div class="input-group">
        <div>
          <input v-model="newMemberId" @blur="newMemberIdValid = newMemberId.length > 0; newMemberIdExist = false" :class="{ 'error': !newMemberIdValid || newMemberIdExist }" placeholder="Enter ID" />
          <p v-if="!newMemberIdValid" class="error-text">ID is required</p>
          <p v-if="newMemberIdExist" class="error-text">ID already exists</p>
        </div>
        <div>
          <input v-model="newMemberPassword" @blur="newMemberPasswordValid = newMemberPassword.length > 0" type="password" :class="{ 'error': !newMemberPasswordValid }" placeholder="Enter Password" />
          <p v-if="!newMemberPasswordValid" class="error-text">Password is required</p>
        </div>
        <div>
          <input v-model="newMemberName" @blur="newMemberNameValid = newMemberName.length > 0" :class="{ 'error': !newMemberNameValid }" placeholder="Enter Name" />
          <p v-if="!newMemberNameValid" class="error-text">Name is required</p>
        </div>
        <div>
          <input v-model="newMemberPhone" @blur="newMemberPhoneValid = newMemberPhone.length > 0" :class="{ 'error': !newMemberPhoneValid }" placeholder="Enter Phone" />
